...
def carregar_dados_completo(caminho):
    ...
    print(f"    Período carregado: {df.index.min()} até {df.index.max()}")
...
def processar_periodo_completo(df_base, alvo):
    print(f"\n>>> Analisando PERÍODO COMPLETO para: {alvo}")
    
    df_atual = df_base.copy()
    
    if alvo not in df_atual.columns:
        print(f"    AVISO: Coluna '{alvo}' não existe. Pulando...")
        return None, None
    ...
    if df_atual.empty:
        print(f"    ERRO CRÍTICO: Sem dados válidos para '{alvo}' (Coluna pode estar vazia/corrompida).")
        return None, None
    ...
    print(f"    Erro Médio (MAE) no período todo: {mae:.2f}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"\n--- SUCESSO! {cont} gráficos foram gerados na pasta atual. ---")
